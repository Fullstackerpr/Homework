...
export class PatientController{
    async singupPatient(req, res){
        try {
            ...
            if(existPhone){
                return catchError(res, 409, 'Phone number is already exists');
            }
            const hashedPassword = await encode(password, 7);
            const patient = await Patient.create({
                fullName, phoneNumber, hashedPassword, address, age, gender
            });
    ...
    async signinPatient(req, res){
        try {
            ...
            const patient = await decode(password, patient?.hashedPassword);
    ...
    async getAllPatientById(req, res){
    ...
    async updatePatientById(req, res){
        try {
            const id = req.params.id;
            const patient = await PatientController.findPatientById(id);
